<template>
  <n-card>
    ...
    >
      ...
      <n-form-item>
        <n-button type="primary" @click="showUploadMedia = true">添加视频</n-button>
      </n-form-item>
    </n-form>
    <n-data-table
      :columns="columns"
  ...
  <!-- 编辑 -->
  <n-modal v-model:show="showEditModal" preset="dialog" title="视频">
    <n-form
      ref="editFormRef"
    ...
    >
      ...
      <n-form-item label="视频地址">
        ...
        <n-button style="margin-left: 20px;" @click="handleCopyUrl">复制</n-button>
      </n-form-item>
    </n-form>
    <template #action>
...
const getData = async () => {
  ...
  const data = await api.media.findAllVideo(page, pageSize, name);
  if (data.code === 0) {
